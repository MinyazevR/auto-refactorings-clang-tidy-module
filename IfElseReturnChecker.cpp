...
/// This function is designed to get the length of the token to handle offsets correctly.
static int getTokenLenght(SourceLocation loc, const clang::ASTContext &context) {
    return clang::Lexer::MeasureTokenLength(loc, context.getSourceManager(), context.getLangOpts());
};
...
static bool isBlockInCurrentIfStmt(const clang::CFGBlock *block,
    ...
    for(auto &&blockElement: block->Elements) {
        ...
        if (stmt->getBeginLoc() > manager->getExpansionLoc(cfgStmtElementStmt->getBeginLoc())
            || stmt->getEndLoc() <  manager->getExpansionLoc(cfgStmtElementStmt->getEndLoc())) {
            return false;
        }
    }
...
/// Checking that the block does not include any meaningful logic other than ReturnStmt
static bool isOnlyReturnBlock(const clang::CFGBlock *block, const SourceManager *manager) {
...
static llvm::StringRef get_source_text(const clang::CharSourceRange &range,
    ...
    return clang::Lexer::getSourceText(
            clang::CharSourceRange::getTokenRange(clang::SourceRange{start, end}), *manager, langOptions);
}
...
static bool isOnlyIterruptionBlock(const clang::CFGBlock *block, 
    ...
    return block->getBlockID() == cfg.getExit().getBlockID() 
            || isOnlyReturnBlock(block, manager) 
            || block->hasNoReturnElement();

}

/// It is used to calculate the weight of the Then or Else Compound Stmt of the main IfStmt, 
// on the basis of which a decision will be made about flipping the if and else bodies
static int getNodeWeight(const Stmt* stmt,
    ...
    return manager->getExpansionLineNumber(end) - manager->getSpellingLineNumber(start);
}

/// When using spaces, it is difficult to understand what Indent is, so the user is prompted 
...
void IfElseReturnChecker::runInternal(IfStmt* ifStmt,
    ...
    bool isThenFirst = getNodeWeight(thenStmt, manager) <= getNodeWeight(elseStmt, manager);
    ...
    std::unique_ptr<clang::CFGStmtMap> stmtToBlockMap(clang::CFGStmtMap::Build(&cfg, parentMap.get()));

    auto context = Result.Context;

    if (mNeedShift) {
        if (!addIterruptionBlockToStmts(targetStmt, cfg, manager, stmtToBlockMap.get(), context)) {
            mNeedShift = false;
        }
    }
    ...
    if (!context || !isThenFirst) {
        ...
        if (auto* compoundElseStmt = dyn_cast<CompoundStmt>(elseStmt)) {
            ...
            auto left = manager->getExpansionLoc(elseLBracLoc).getLocWithOffset(getTokenLenght(elseLBracLoc, *context));
            auto right = manager->getExpansionLoc(elseRBracLoc).getLocWithOffset(-1);

            auto elseText = Rewrite.getRewrittenText(CharSourceRange::getTokenRange(left, right));
                // : get_source_text(CharSourceRange::getTokenRange(left, right), manager, context).str();

            // auto mainIndent = clang::Lexer::getIndentationForLine(ifStmt->getBeginLoc(), *manager);
        ...
        if (auto* compoundThenStmt = dyn_cast<CompoundStmt>(thenStmt)) {
            ...
            clang::SourceRange range(manager->getExpansionLoc(compoundThenStmt->getRBracLoc().getLocWithOffset(1)),
...
bool IfElseReturnChecker::reversCondition(const IfStmt *ifStmt,
    ...
    if (auto then = dyn_cast<CompoundStmt>(ifStmt->getThen())) {
        ...
        mFixList.push_back(FixItHint::CreateInsertion(expansionEndLoc.getLocWithOffset(-1), ")"));
        return true;
    }
...
/// We collect all the blocks that are the boundary of the Stmt, starting with exit. 
/// If a block is found that is not included in this Stmt, but is achievable from it, 
/// as well as meaningful, the analysis stops, as this means that there are execution paths outside the Stmt,
/// which prevents it from being interrupted in advance with a conservative approach. 
/// Otherwise, the block is returned, which must be tightened to interrupt the desired branch.
static const clang::CFGBlock * getItteruptBlockForStmt(
                            const clang::CFG &cfg,
                            const clang::SourceManager *manager,
                            const Stmt* stmt,
                            const clang::ASTContext *context,
    ...
    for(auto &&block: frontier) {
        ...
        for (auto blockPred: block->preds()) {
            auto isBlockInCurrentStmt = isBlockInCurrentIfStmt(blockPred, stmt, manager);
...
/// Adds to the source text the text of the block that needs to be tightened to interrupt the branch.
void IfElseReturnChecker::appendStmt(const CompoundStmt* stmt,
    ...
    auto ref = get_source_text(manager->getExpansionRange(stmtToAdd->getSourceRange()),
                                manager, context);
    ...
    auto indent = clang::Lexer::getIndentationForLine(manager->getExpansionLoc(lastStmt->getBeginLoc()), *manager);
    ...
    SourceLocation semi_loc(
        ...
        last_token_loc, clang::tok::semi, *manager, context->getLangOpts(), false));

    if (semi_loc.isInvalid()) {
        semi_loc = clang::Lexer::getLocForEndOfToken(last_token_loc, 0, *manager, context->getLangOpts());
    }
...
/// It's not the most efficient way to move a block to the left by the specified number of characters. 
void IfElseReturnChecker::indentBlock(const CompoundStmt* stmt, 
    ...
    copy(vector.begin(), vector.end(), std::ostream_iterator<std::string>(inputStream, "\n"));
...
static clang::CharSourceRange getCompoundStmtRange(const CompoundStmt *stmt, 
                                const clang::ASTContext &context,
                                const SourceManager *manager,
    ...
    auto leftBorder = manager->getExpansionLoc(LBracLoc)
                                .getLocWithOffset(getTokenLenght(LBracLoc, context));
...
void IfElseReturnChecker::reverseStmt(const IfStmt* ifStmt,
    ...
    if (!compoundElseStmt) {
        ...
        SourceLocation semi_loc(
        clang::Lexer::findLocationAfterToken(
            elseEndLocation, clang::tok::semi, *manager, context.getLangOpts(), false));

        if (semi_loc.isInvalid()) {
            semi_loc = elseEndLocation;
        }

        elseTokenRange = CharSourceRange::getTokenRange({elseStartLocation, semi_loc});
    } else {
    ...
    //                             .getLocWithOffset(getTokenLenght(ifLBracLoc, context));
    // auto rightBorderOfThen = manager->getExpansionLoc(ifRBracLoc);

    // auto indent = clang::Lexer::getIndentationForLine(rightBorderOfThen, *manager);
    // auto offset = indent.size() + 1;
    // auto startOfRBracStringLoc = rightBorderOfThen.getLocWithOffset(-offset);
    // auto str = Rewrite.getRewrittenText({startOfRBracStringLoc,
    //                             rightBorderOfThen.getLocWithOffset(-1)});

    // auto ifTokenRange = CharSourceRange::getTokenRange(leftBorderOfThen.getLocWithOffset(1),
    //                                      rightBorderOfThen.getLocWithOffset(-1));
    ...
    if (needShift) {
        ...
        if (compoundIfStmt) {
            auto elseLength = getTokenLenght(ifStmt->getElseLoc(), context);
            clang::SourceRange range(manager->getExpansionLoc(compoundIfStmt->getRBracLoc().getLocWithOffset(1)),
                    ifStmt->getElseLoc().getLocWithOffset(elseLength));
...
bool IfElseReturnChecker::addIterruptionBlockToStmts(const Stmt* stmt,
    ...
    if (auto block = getItteruptBlockForStmt(cfg, manager, stmt, context, stmtToBlockMap)) {
        auto *iterruptionBlockStmt = getIterruptStatement(block);
        appendStmt(dyn_cast<CompoundStmt>(stmt), iterruptionBlockStmt, manager, context);
        return true;
    }
