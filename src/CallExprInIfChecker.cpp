...
CallExpInIfChecker::CallExpInIfChecker(
...
void CallExpInIfChecker::check(const MatchFinder::MatchResult &Result) {
    ...
    if (!IfStmtNode->getCond()->getSourceRange()
        ...
        if (const auto *ParentStmt = Parent.get<Stmt>()) {
            if (const auto *ParentIfStmt = dyn_cast<IfStmt>(ParentStmt)) {
                if (ParentIfStmt->hasElseStorage()
                    && ParentIfStmt->getElse()->getID(*AstContext) == IfStmtNode->getID(*AstContext)) {
                        llvm::errs() << "IS IF ELSE\n";
                        return;
                }
            }
        }
    ...
    if (Pattern != "*") {
      static llvm::Regex Regex(Pattern);
      auto CalleeStringName = CallExpr->getCalleeDecl()->getAsFunction()->getNameAsString();
      if (!Regex.match(CalleeStringName)) {
        return;     
      }   
    }
    ...
    if (ReturnTypePattern != "*") {
        static llvm::Regex Regex(ReturnTypePattern);
        if (!Regex.match(ReturnTypeString)) {
            return;
        }
    }
    ...
    auto CallExprSourceCode  = clang::Lexer::getSourceText(
            clang::CharSourceRange::getTokenRange(CallExpr->getSourceRange()), *Manager, AstContext->getLangOpts());

    auto VariableName = VariablePrefix + std::to_string(VariableCounter);
    VariableCounter += 1;

    auto Str = IfStmtIndent.str() 
                + ReturnTypeString + " " 
                + VariableName + " = "
                + CallExprSourceCode.str() + ";\n";

    auto Diag = diag(CallExpr->getBeginLoc(), "It looks like you are using function call in the if condition.");
    Diag << FixItHint::CreateInsertion(IfStmtNode->getBeginLoc().getLocWithOffset(-1), Str);
    Diag << FixItHint::CreateReplacement(CallExpr->getSourceRange(), VariableName);
}
